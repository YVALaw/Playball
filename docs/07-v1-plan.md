...
build** — the reporter tests on BlueStacks, where a browser tab over a
LAN IP is not a secure context and Chrome will only ever hand him a
shortcut. An APK installs into BlueStacks directly and is a truer read
of the Android target than Chrome is, so the shell came early. The rest
of the tail (17 store, 19 ship) keeps its order.
...
**Decisions taken at the door:**

- **`appId` is `com.playball.dynasty`**, named in a committed
  `capacitor.config.ts` rather than left in a generated file. It is the
  package name Android identifies the install by and cannot be changed
  once a listing exists.
- **`androidScheme: 'https'`**, so the WebView origin is a secure
  context. A career lives in IndexedDB, and some storage APIs are
  reserved for secure origins — an http scheme quietly costs the save
  system its footing.
- **`android/` is not committed.** It is generated by `cap add` and
  refreshed by `cap sync` from `dist/` on every build; committing it
  duplicates the app and turns each regenerate into a diff nobody
  reads. The keystore is ignored twice over — lose it and the app can
  never be updated on a listing again; stage 19 owns backing it up.
