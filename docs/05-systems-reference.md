...
| 10d | **Your own home state is worth about 3% more fit than a rival gets in his, at one star, falling to nothing at five.** `pitchFor` is handed the `pipelineStrength` callback only for the user, so his home state reads 60 of network (scale 1.15) where the other ninety-five fall to the literal 45 (scale 1.00). Reviewed and **kept deliberately on September 6 2026**: the earned half of the pipeline system — a coordinator's market, a state signed into repeatedly — is a decision only the user can make, since rivals carry no economy at all, and this is the same shape as the recruiting-skill gap in §16.4. Reach is unaffected and already symmetric: rivals call `canPursue` with a boolean for their own state and get the same one-star bonus. `06` §AA has the measurement and the change that was declined. | Nothing. It is inside the fit number. | `pipelineStrength` — `engine/economy.ts`; `fit` — `engine/recruiting.ts`; `06` §AA | SHIPPED |
...
| 21 | **A freshman under 52 overall has a 14% chance of a second, large headroom draw**, 24±9 on top of ordinary headroom. It fires for freshmen only, so it reaches about 8% of a recruiting class and about 1.8% of a roster three quarters upperclassmen. It was the only channel that could make a gem until headroom itself became a roll — row 21b. | Nothing at all. | `projectPotential` — `engine/players.ts`; §73.2 | SHIPPED |
...
| 21b | **Headroom is a roll, and how wide it is depends on how raw the man already is.** `reach` runs 3.0 at 44 overall down to 1.6 at 64, and it sets both the roof of the draw — `room × reach` — and the exponent that shapes it, so a raw eighteen year old rolls against three times his class's room and a finished one against very nearly the band he always had. The mean stays exactly `room` at every reach (FR 11, SO 7, JR 4, SR 2, the old band means) by construction rather than by tuning, so the league's total talent does not move. Before this, headroom came off class year and nothing about the player: a ceiling was current ability plus a constant, two adjacent gap bands held 72.6% of a class, and an A potential grew like a C. | A potential on the card, and men who grow at visibly different rates. | `projectPotential`, `ROOM`, `REACH_RAW`, `REACH_FINISHED` — `engine/players.ts`; §73.3 | SHIPPED |
| 21c | **19% of the men who are already finished roll on a bust lobe instead**, which tops out at 15% of their roof — nought to two and a half points for a freshman. `polish²` keeps it off the raw kids, because a forty overall freshman with a forty ceiling is a bug rather than a prospect, and the other lobe's exponent is solved so the mean is still `room` and the roof never moves. Five stars who arrive already finished went 2.2% to 10.6%. | A blue chip who never develops, and nothing anywhere saying he was always going to be one. | `BUST`, `BUST_TOP` — `engine/players.ts`; §75.2 | SHIPPED |
| 21d | **The nine points below the cap are a landing strip, not a wall.** Below 85 a ceiling is untouched; above it the distance left is spent exponentially, so an uncapped 88 lands at 87.6, a 94 at 90.7, a 110 at 93.4, and nothing ever arrives. Row 21a's clamp is what made the best grade in the game common — 9.9 men a class came out at exactly 94, three quarters of every S, because truncation is what makes a top dense. S per class 12.4 to 5.5, men at the cap 9.9 to 0.2, and the lowest overall carrying an S is 30. Read off `GENERATED_POTENTIAL_CAP` rather than written down, so the two can never disagree. | An S that is rare, and a top grade that tells a generational arm from a merely excellent one. | `SOFT_LANDING`, `land` — `engine/players.ts`; §75.3 | SHIPPED |
...
| 113 | **Five days sit between the postseason's stages, and the number is what `recoveryGap` asks of a starter who threw over ninety pitches.** Before it existed the regionals opened the day after the last conference game and not one of the thirty-two teams in the field had its number one or number two available — zero of thirty-two on both slots, uniform rather than unfair, which is exactly why nothing caught it. Sixty-four of sixty-four clubs now carry all four slots into a regional opener. Every tournament in a stage also opens on the same night instead of the stages playing end to end, so no conference is permanently handed the short rest and June spans 31 days against 106. | A calendar, and a June pitched by the arms a coach would pick. | `STAGE_BREAK`, `onTheSameNights` — `engine/postseason.ts`; §69.1 | SHIPPED |
| 114 | **Where a school's colour has to be read as text it is not the school's colour.** All ninety-six fall below 4.5:1 on the dark theme's paper and eighty-seven below 3:1 — the deep navies and forest greens are not low contrast there, they are gone — so `teamInk` keeps the hue and walks the lightness until it clears. Both cuts are handed down as custom properties rather than one colour returned, because an inline colour gives one theme the other's answer. Worst case 4.60, hue held inside twelve degrees. | Club colours on the pregame card that look like the clubs' colours and can be read. | `teamInk` — `ui/accent.ts`; §69.4 | SHIPPED |
| 115 | **A short season is a shorter weekend, not a shorter schedule.** The eleven-series conference round robin and all twelve crossover games survive at 34 games exactly as they do at 45 — dropping rounds would judge two programs on different opponents, and sealing the eight leagues off would leave RPI nothing to compare. What comes out is the third game of a weekend, and the midweek arm starts in the slot after the weekend's rather than a hard-coded third, so a short year is a three man rotation instead of a four man one with a hole in it. A standard world is handed `DEFAULT_SEASON` itself rather than a copy, so every calibration figure ever taken still measures the league it was taken from. | Two chips on the rules fold: "Three-game weekends, four-man rotation" and "Two-game weekends, three-man rotation". | `SEASON_SPANS`, `configForRules`, `seriesGames`, `buildSchedule` — `engine/season.ts`; §70.2 | SHIPPED |
| 116 | **A drafted man's professional career is derived, never stored and never drawn** — four answers a summer, read out of four byte ranges of one hash of his id and the year, so the same man always lives the same life. Three of the four were not rolls: the old `h × 31 + c` barely moved its high bits when the year did, so a man was stamped once for life — an All-Star every summer or never one, 39 men of 400 in all twenty seasons and the other 361 in none, and promoted every year he was eligible or never. FNV-1a with an avalanche now, masked to 31 bits because the callers read it with a signed shift and a negative value passes every roll. The All-Star coin reads his draft-day talent as well, `talent − 9` capped at 26%, where it was a flat 9% that consulted nothing: 11.2% of the men who reach the top level ever make a team, at 1.18 summers apiece, and 16.3% of drafted men reach it at all against 39% before. | One line a year on an alumnus's card. | `hash`, `bigLeagueSummer`, `proCareer` — `engine/legacy.ts`; §74 | SHIPPED |
...
**The S row is out of date, and the rows around it may be.** Measured again
while §73 was written, the 92 floor was holding 8.4 men a class before any of
that work, 12.4 once headroom became a roll, and 5.5 once the landing strip
replaced the cap's wall — §73.5 and §75.4. Nothing else in the table has been
re-measured since, and this document does not guess at what it has not counted.
...
`projectPotential` — the 14% of freshmen under 52 overall who get a second large
headroom roll, and for a long time the only reason a three star could become a
star — **survives the cap untouched**. It was the thing most at risk, since squeezing the top of the
