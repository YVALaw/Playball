    ...
    <main className="module-workspace god-desk">
      ...
      {panel === 'move' && (
        <section className="god-card">
          ...
          <div className="god-actions">
            <button type="button" className="tap" disabled={!moveTarget} onClick={() => { if (!moveTarget) return; if (movePlayer(man.id, moveTarget.index)) { setNote(`${man.name} is at ${moveTarget.def.school} now.`); setMoveTo(-1); } }}>MOVE PLAYER</button>
            <SureButton label="CUT PLAYER" onSure={() => { if (cutPlayer(man.id)) closeGod(); }} />
          </div>
          <p className="god-note">A move changes this player's program, not league alignment. League membership is controlled from the Leagues tool.</p>
        </section>
      )}
